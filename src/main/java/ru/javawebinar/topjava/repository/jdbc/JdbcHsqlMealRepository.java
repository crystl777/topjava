...
public class JdbcHsqlMealRepository extends JdbcMealRepository {
    ...
    @Override
    public List<Meal> JdbcTemplateQueryList(JdbcTemplate jdbcTemplate, LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        return jdbcTemplate.query(
                "SELECT * FROM meals WHERE user_id=?  AND date_time >=  ? AND date_time < ? ORDER BY date_time DESC",
                JdbcMealRepository.ROW_MAPPER, userId, Timestamp.valueOf(startDateTime), Timestamp.valueOf(endDateTime));
    }
}
